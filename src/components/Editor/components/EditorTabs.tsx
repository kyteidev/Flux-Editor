      ...
      <div class="flex max-h-full min-h-full w-full max-w-full select-none space-x-1 overflow-x-auto overflow-y-hidden p-1 pb-0">
        <For each={tabs().map((t) => t[0])}>
          {(tabName, index) => {
            ...
            return (
              <div
                class={`${activeTab() === index() ? "bg-base-100" : "bg-base-200"} group mr-1 flex h-8 max-w-72 items-center rounded px-2 py-1 text-center hover:bg-base-50 active:brightness-125`}
                onclick={() => {
                  if (!isClosed()) {
                    setActiveTab(index());
                    open();
